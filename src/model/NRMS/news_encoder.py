...
class NewsEncoder(torch.nn.Module):
    def __init__(self, config, pretrained_word_embedding):
        ...
        self.subcategory_emb = nn.Embedding(config.num_subcategory, config.category_embedding_dim, padding_idx=0)
        self.subcategory_dense = nn.Linear(config.category_embedding_dim, config.num_attention_heads)
    ...
    def forward(self, news):
        ...
        # print(f"final_news_vector shape: {final_news_vector.shape}")
        # print(f"category_vecs shape: {category_vecs.shape}")
        
        # all_vecs.append(category_vecs)
        # start += 1
        
        if 'category' in news and news['category'] is not None:
            category = news['category'].to(device).long()
            category_vecs = self.category_dense(self.category_emb(category))
            all_vecs.append(category_vecs)
        
        if 'subcategory' in news and news['subcategory'] is not None:
            subcategory=news['subcategory'].to(device).long()
...
# import torch.nn.functional as F
# from model.general.attention.multihead_self import MultiHeadSelfAttention
# from model.general.attention.additive import AdditiveA
# 
# 
# 
# ttention
